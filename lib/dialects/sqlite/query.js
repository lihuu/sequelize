...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    ...
    this.sql = sql;
    if (this.sequelize.config.password&&!this.connection.encryp&&sql.startsWith('CREATE TABLE IF NOT EXISTS')) {
      //只有在自动创建表的时候，可能会有加密的问题，后面的操作就没有什么问题了啦
      // Make it possible to define and use password for sqlite encryption plugin like sqlcipher
      this.connection.encryp = true;
      conn.run(`PRAGMA KEY=${this.sequelize.escape(this.sequelize.config.password)}`);
    }
